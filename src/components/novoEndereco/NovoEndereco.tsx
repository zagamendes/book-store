...
const NovoEndereco: React.FC<NovoEnderecoProps> = ({ closeModal }) => {
  ...
  const [endereco, setEndereco] = useState({} as ViacepResponse);
  const [complemento, setComplemento] = useState({} as complementoProps);

  const getAddressInfo = async () => {
    console.log("aqui");

    const { data } = await axios<ViacepResponse>(
      ...
      enderecos: [
        {
          ...
          id: `${endereco.logradouro}-${complemento.numero}-${
            complemento.complemento ? complemento.complemento : "complemento"
  ...
  const updateAddress = async () => {
    const novoEndereco: enderecoDeEntrega = {
      bairro: endereco.bairro,
      cep: cep,
      localidade: endereco.localidade,
      logradouro: endereco.logradouro,
